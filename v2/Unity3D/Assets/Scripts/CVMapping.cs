...
public class CVMapping : MonoBehaviour
{
    ...
    public System.Text.StringBuilder saveORBImage(Mat camMat,  Texture2D tex, int height, int nKeyPoints, string filename){
        Mat afterMat = new Mat();

        KeyPoint[] keyPoints = this.getKeyPoints(camMat, nKeyPoints*10);
        Cv2.DrawKeypoints(camMat, keyPoints, afterMat, new Scalar(0, 0, 255), 0);

        var aTex = this.MatToTexture(afterMat, tex);
