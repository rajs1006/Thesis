...
public class ImageSynthesis : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        //count = new SampleCountController();
    ...
    public void PointAndPnP(Point2f[] imgPts, Point3f[] objPts, string folder, string filename)
    {
        PointAndPnP(imgPts, objPts, Path.Combine(folder, filename));
    }

    public void PointAndPnP(Point2f[] imgPts, Point3f[] objPts, string filename)
    {
        openCV.pnp(objPts, imgPts, capturePasses[0].camera, SolvePnPFlags.Iterative, filename);    
        
    }

    public void writeCameramatrix(){
    ...
    public void Save(Vector3[] transformed3dKeypoints, Bounds objBound, string filename, int width = -1, int height = -1, string path = "",
     bool saveOnlyImageAndLayer = false, bool generate = false, bool control = false)
    {
        if (width <= 0 || height <= 0)
        {
            width = Screen.width;
        ...
        StartCoroutine(
            WaitForEndOfFrameAndSave(pathWithoutExtension, filenameExtension, width, height, saveOnlyImageAndLayer, transformed3dKeypoints, objBound, generate, control));
    }

    private IEnumerator WaitForEndOfFrameAndSave(string filenameWithoutExtension, string filenameExtension, int width, int height, bool saveOnlyImageAndLayer, Vector3[] transformed3dKeypoints, Bounds objBound, bool generate, bool control)
    {
        yield return new WaitForEndOfFrame();
        Save(filenameWithoutExtension, filenameExtension, width, height, saveOnlyImageAndLayer, transformed3dKeypoints, objBound, generate, control);
    }

    private void Save(string filenameWithoutExtension, string filenameExtension, int width, int height, bool saveOnlyImageAndLayer, Vector3[] transformed3dKeypoints, Bounds objBound, bool generate, bool control)
    {
        if (saveOnlyImageAndLayer)
        {
            var passImg = capturePasses[0];
            Save(passImg.camera, filenameWithoutExtension + passImg.name + filenameExtension, width, height, passImg.supportsAntialiasing, passImg.needsRescale, transformed3dKeypoints, objBound, generate, control);
        ...
        {
            foreach (var pass in capturePasses)
                Save(pass.camera, filenameWithoutExtension + pass.name + filenameExtension, width, height, pass.supportsAntialiasing, pass.needsRescale, transformed3dKeypoints, objBound, generate, control);
        }
    }

    private void Save(Camera cam, string filename, int width, int height, bool supportsAntialiasing, bool needsRescale, Vector3[] transformed3dKeypoints, Bounds objBound, bool generate, bool control)
    {
        ...
        if(generate){ 
            if(nKeyPoints == 0){
                ...
                Debug.Log($"kpLen  {kpLen}");
                KeyPoint[] keyPoints = openCV.getKeyPoints(camMat, kpLen);
                ...
                openCV.saveORBImage(camMat, tex, height, nKeyPoints, filename);
                ...
                using (TextWriter kpWriter = new StreamWriter(keypointFileName))
                {
                    foreach(var kp3D in transformed3dKeypoints){
                        ...
                        kpWriter.WriteLine("("+gdKeyPoints.x + ", " + gdKeyPoints.y + ") : " + (kp3D));
                    }
                    kpWriter.Flush();
                }
        ...
        }else if (nKeyPoints != 0)
        {
            ...
            {
                var imgPts = new Point2f[nKeyPoints];
                var objPts = new Point3f[nKeyPoints];
                ...
                foreach (var kp3D in transformed3dKeypoints)
                {
                    ...
                    Vector3 gdKeyPoint = cam.WorldToScreenPoint(kp3D);
                    gtKeyPoints.AppendLine(gdKeyPoint.x + "," + (height - gdKeyPoint.y));
                    gdKpWriter.WriteLine(gdKeyPoint.x + "," + (height - gdKeyPoint.y));

                    imgPts[i] = new Point2f(gdKeyPoint.x, gdKeyPoint.y);
                    objPts[i] = new Point3f(kp3D.x, kp3D.y, kp3D.z);
                    i++;

                }

                this.PointAndPnP(imgPts, objPts, filename.Replace(Path.GetExtension(filename), ""));
            ...
            System.Text.StringBuilder cvKeyPoints = openCV.saveORBImage(camMat, tex, height, nKeyPoints, filename);
